...
import GeneralLeaderboard from '../services/generalLeaderboard.service';
...
class LeaderboardController {
  ...
  static getGeneralLeaderboard = async (_req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await GeneralLeaderboard.returnResponse();
      return res.status(200).json(result);
    } catch (error) {
      next(error);
    }
